...
<section>
    <script type="text/javascript">
        SimpleJekyllSearch({
            ...
            searchResultTemplate: '<div class="search-title"><a href="{url}"><h3> {title}</h3></a><p class="meta">{date} <div class="right"><i class="fas fa-tag"></i> {tags}</div></p><p>{% include read-time.html content=post.content %}</p><p>{excerpt}</p></div><hr>',          
